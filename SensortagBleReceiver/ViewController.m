...
- (void) peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error
{
    ...
    int frequency = 0x0A;//0x64; //センサデータの取得頻度 [10~1000]で設定
    ...
    for (CBCharacteristic *characteristic in service.characteristics) {
        ...
        } else if ( [characteristic.UUID isEqual:[CBUUID UUIDWithString:UUID_MOV_CONF]]){ //モーションセンサ（加速度・角速度・地磁気）
            // モーションセンサの初期設定(センサの起動)のために、16bitの整数値(unsigned short)を送信
            // 各センサに該当するbitにセンサのon or off(つまり0 or 1)を記述
            // [参考資料]
